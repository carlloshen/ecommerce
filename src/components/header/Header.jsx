...
function ButtonsLoginRegister({ handleClickPageChange, classNameButtonSignUp, classNameButtonLogin }) {
  return (
    <div className="flex justify-between md:justify-center items-center gap-[1vw]">
      <Link to={"/cadastro"}>
        <button id="cadastro" className={classNameButtonSignUp} onClick={() => handleClickPageChange()}>
          Cadastre-se
        </button>
      </Link>
      <Link to={"/login"}>
        <button id="login" className={classNameButtonLogin} onClick={() => handleClickPageChange()}>
          Entrar
        </button>
      </Link>
...
function Header() {
  ...
  return pathname === "/login" ? null : pathname === "/cadastro" ? null : (
    <header className="bg-blue-900 items-center pt-5 pb-5 flex flex-col lg:grid-rows-2  w-screen ">
      <div className="flex justify-between flex-col lg:justify-start lg:flex-row  items-center h-20  w-[90vw] lg:max-w-[1440px]">
          ...
          <div className="ml-5 mr-5 mt-5">
            ...
            <nav className="flex flex-col mb-10 text-stone-500">
              <Navbar
                ...
                className={"font-semibold hover:bg-slate-300 p-2 rounded-md cursor-pointer"}
              />
            </nav>
            <hr className="mb-3" />
            <ButtonsLoginRegister
              ...
              classNameButtonSignUp={
                "text-stone-500 lg:none font-semibold hover:bg-slate-300  text-sm pl-3 pr-3 min-w-[30px] h-[30px] rounded-md lg:text-white"
              }
              classNameButtonLogin={
                "bg-blue-700 lg:none rounded-md text-white w-[30vw] max-w-[150px] h-[30px] text-sm front-bold hover:bg-blue-900"
              }
            />
          </div>
        </aside>
        ...
        <div className="flex w-full  justify-between">
          <button className="lg:hidden" onClick={() => handleShowMenuHamburguer(true)}>
          ...
          <div className="lg:order-1 hidden lg:block">
            <ButtonsLoginRegister
              handleClickPageChange={handleClickPageChange}
              classNameButtonSignUp={
                "hidden text-white lg:block font-semibold text-sm pl-3 pr-3 min-w-[30px] h-[30px] rounded-md"
              }
              classNameButtonLogin={
                "hidden lg:block rounded-md text-white w-[7vw] h-[30px] text-sm front-bold lg:hover:bg-orange-600 bg-orange-500"
              }
            />
          </div>
        </div>
